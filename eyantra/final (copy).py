...
def coins_contours(img):
	...
	_,carray, hierarchy = cv2.findContours(cont_img, cv2.RETR_EXTERNAL,cv2.CHAIN_APPROX_NONE)
	contours=list();
	for i in range(0,len(carray)):
		a=cv2.contourArea(carray[i]);
		if(a>1000):
			cv2.drawContours(img, carray, i, (0,0,255), 3)
			contours.append(carray[i]);
			#contours.remove(i);
...
def find_gold_coins(img):
	...
	for i in range(0,len(contours)):
		...
		for a,b in mp1:
			if(a>=x and a<=x+width and b>=y and b<=y+height):
				bl,gr,rd=mask_inv[b,a]
				if(bl>0 and gr>0 and rd>0):
					arr.append(mp[(b,a)])
	...
	for i in range(0,len(roi)):
		...
		for ii in range(0,len(contours_in_roi_gold)):
			...
			if(aa>400 and aa<4000):
				if(aa>400 and aa<2000):
					coins[i]+=1;
...
def find_max_gold_coins(img):
	...
	for i in range(0,len(roi)):
		...
		for ii in range(0,len(contours_in_roi_gold)):
			...
			if(aa>400 and aa<6000):
				
				if(aa>400 and aa<2000):
					coins[i]+=1;
					
				if(aa>=2000 and aa<6000):
					coins[i]+=2;
